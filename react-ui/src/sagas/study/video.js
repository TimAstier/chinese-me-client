...
export function* isDataLoaded(id) {
  yield put(studyActions.setCurrentVideoId(id));
  const currentElement = yield select(selectors.getCurrentVideo);
  return (currentElement === undefined) ? false : true;
}

export function* fetchData(episodeId) {
  return yield call(fetchEntities, ['/episode/' + episodeId + '/videos']);
